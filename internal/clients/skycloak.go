...
import (
	...
	clusterv1beta1 "github.com/decade-eng/provider-skycloak/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/decade-eng/provider-skycloak/apis/namespaced/v1beta1"
)
...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal skycloak credentials as JSON"
	errMissingAPIKey        = `required credential "api_key" is missing or empty (SKYCLOAK_MANAGEMENT_KEY)`

	// keyAPIKey is both the key in the credentials secret JSON and the
	// Terraform provider configuration attribute for the Skycloak API key
	// (stored as SKYCLOAK_MANAGEMENT_KEY in Infisical). The optional endpoint
	// and api_version provider attributes are also read from the same secret
	// JSON when present.
	keyAPIKey = "api_key"
)

// TerraformSetupBuilder builds a terraform.SetupFn that returns the Terraform
// provider setup configuration for the in-process (plugin framework)
// architecture.
func TerraformSetupBuilder(frameworkProvider fwprovider.Provider) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if creds[keyAPIKey] == "" {
			return terraform.Setup{}, errors.New(errMissingAPIKey)
		}

		// Set credentials in the Terraform provider configuration. endpoint
		// and api_version are optional provider attributes carried in the
		// same secret JSON.
		ps.Configuration = map[string]any{
			keyAPIKey: creds[keyAPIKey],
		}
		for _, key := range []string{"endpoint", "api_version"} {
...
func resolveModern(ctx context.Context, crClient client.Client, mg resource.ModernManaged) (*namespacedv1beta1.ProviderConfigSpec, error) {
	...
	if !ok {
		...
		return nil, errors.Errorf("provider config type %T is not a client.Object", pcRuntimeObj)
	}
