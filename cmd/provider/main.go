...
import (
	...
	apisCluster "github.com/decade-eng/provider-skycloak/apis/cluster"
	apisNamespaced "github.com/decade-eng/provider-skycloak/apis/namespaced"
	"github.com/decade-eng/provider-skycloak/config"
	"github.com/decade-eng/provider-skycloak/internal/clients"
	controllerCluster "github.com/decade-eng/provider-skycloak/internal/controller/cluster"
	controllerNamespaced "github.com/decade-eng/provider-skycloak/internal/controller/namespaced"
	"github.com/decade-eng/provider-skycloak/internal/features"
	"github.com/decade-eng/provider-skycloak/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Skycloak").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-skycloak"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-skycloak",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Skycloak APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Skycloak APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-skycloak:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Skycloak controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Skycloak controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Skycloak controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Skycloak controllers")
	}
